...
Также и вам предстоит проверить чужой код.
Ознакомившись с разными способами решения одной и той же задачи, вы сможете лучше понять изучаемую тему.
...
Обновленная страница доступна по другой ссылке.
К сожалению, в процессе изменений они случайно внесли баг в форму регистрации.

Попробуйте запустить код из предыдущего шага, указав в качестве начальной страницы новую ссылку.
Если ваш тест упал с ошибкой NoSuchElementException, это означает,
что вы выбрали правильные селекторы и смогли обнаружить баг, который создали разработчики.
Это хорошо! Значит, ваши тесты сработали как надо.
...
Убедитесь что расширение файла .py а не .txt, иначе велика вероятность, что ваше решение не запустится у других.
Убедитесь, что в вашем коде не указаны пути до хромдрайвера, иначе ваш код не запустится у других студентов.
Если вы используете Firefox или что-то, что мы не проходили в курсе, добавьте комментарии о том,
как запустить такой код. Помните, что ваш код будут проверять, в том числе, новички.

Не забудьте посмотреть и отрецензировать работы других учащихся,
только после этого вы получите баллы за данное задание.
