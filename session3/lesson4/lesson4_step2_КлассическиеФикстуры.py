...
Одно из распространенных применений фикстур — это подготовка тестового окружения и очистка тестового окружения и
данных после завершения теста.
...
(документация в PyTest: https://docs.pytest.org/en/latest/how-to/xunit_setup.html?highlight=teardown).
...
создание экземпляра браузера и его закрытие только один раз для всех тестов первого тест-сьюта и
создание браузера для каждого теста во втором тест-сьюте.
Сохраните следующий код в файл s3_lesson4_step2_fixture1_1.py  и запустите его с помощью PyTest.
Не забудьте указать параметр -s, чтобы увидеть текст, который выводится командой print().

# pytest -s -v s3_lesson4_step2_fixture1_1.py: (создание экземпляра браузера и его закрытие только один раз для всех тестов первого тест-сьюта)
# pytest -s -v s3_lesson4_step2_fixture1_2.py: (создание экземпляра браузера и его закрытие для каждого теста)


# pytest -s s3_lesson4_step2_fixture1_1.py   (создание экземпляра браузера и его закрытие только один раз для всех тестов первого тест-сьюта)
...
# s3_lesson4_step2_fixture1_2.py: (создание экземпляра браузера и его закрытие для каждого теста)
class TestMainPage2():
...
Возможно, вы захотите оптимизировать время прогона тестов, но лучше это делать с помощью других инструментов,
